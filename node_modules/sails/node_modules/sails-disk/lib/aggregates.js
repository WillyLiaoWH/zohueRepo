...
var Aggregate = module.exports = function(options, results) {
  ...
  if(!options.sum && !options.average && !options.min && !options.max) {
    return { error: Errors.InvalidGroupBy };
  }
...
Aggregate.prototype.group = function(groupBy, results) {
  // console.log('-->', groupBy, require('lodash').where(results,{'type':'groupBy test'}));
  // console.log('-->', groupBy, results);//require('lodash').where(results,{'type':'groupBy test'}));
  // if (groupBy&&groupBy[0]&&groupBy[0] === 'type'&&results[0].type==='groupBy test') {
  //   console.log('the groupby!!',groupBy, results);
  // }
